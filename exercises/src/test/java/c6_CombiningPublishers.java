...
public class c6_CombiningPublishers extends CombiningPublishersBase {
	 ...
	 * Um retorna Mono e o outro Flux
     */
    @Test
    ...
    public Mono<String> chooseSource() {
		return Mono.defer(() -> {
			if ("A".equals(sourceRef.get())) return sourceA(); //<- choose if sourceRef == "A"
			if ("B".equals(sourceRef.get())) return sourceB(); //<- choose if sourceRef == "B"
			return Mono.empty(); //otherwise, return empty
		});
    }

	// Sempre que houver uma nova subscrição o Mono será executado novamente, por isso precisa do `defer()`
	// Se não tiver, só vai ser avaliado quando o método `chooseSource` for executado
    @Test
    ...
    @Test
    public void cleanup() {
		// Não funciona na JDK 21
        // BlockHound.install(); //don't change this line, blocking = cheating!

		// O Flux.usingWhen() é usado para trabalhar com recursos que precisam ser gerenciados de forma assíncrona
