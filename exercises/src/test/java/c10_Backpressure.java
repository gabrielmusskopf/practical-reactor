...
public class c10_Backpressure extends BackpressureBase {
    ...
    public void limited_demand() {
		/*
		limitRate(N) splits the downstream requests so that they are propagated upstream in smaller batches. For instance,
		a request of 100 made to limitRate(10) would result in, at most, 10 requests of 10 being propagated to the upstream
		 */
        CopyOnWriteArrayList<Long> requests = new CopyOnWriteArrayList<>();
    ...
    public void uuid_generator() {
		Flux<UUID> uuidGenerator = Flux.create(sink -> sink.onRequest(r -> {
			// Sink por que vai gerar elementos dinamicamente
			// onRequest para executar a cada request
			for (int i = 0; i < r; i++) {
